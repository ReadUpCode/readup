...
var getTitleText = function(body, res, suggestedTags) {
  ...
  for (var j = 0; j < titleArray.length; j++) {
    if (titleArray[j] in topTags) {
      suggestedTags[topTags[titleArray[j]]] = topTags[titleArray[j]];
    }
  }
    ...
    if (!error && response.statusCode === 200){
      ...
      for (var i = 0; i < cleanKeywords.length; i++) {
        if (cleanKeywords[i] in topTags) {
          suggestedTags[topTags[cleanKeywords[i]]] = topTags[cleanKeywords[i]];
        }
      }
      request.get(req.body.url, function(error, response, body) {
        if(error) {throw error;}
        if(!error && response.statusCode === 200) {
          getTitleText(body, res, suggestedTags);
        }
      });
    }
  });
